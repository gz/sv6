...
#define CHECK_STAGE()                                   \
  ...
  char file[64];
  snprintf(file,  64, "/maponly_fd_%d", cpu);

  int fd = open(file, O_CREAT | O_RDWR, 0);
  assert(fd > 0);

  void* buf = calloc(1, 0x1000);
  int ret2 = write(fd, buf, 0x1000);
  assert(ret2 == 0x1000);
  fsync(fd);
  ...
  {
    volatile char *p = base + cpu * npg * 0x10000000000;
